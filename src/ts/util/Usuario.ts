...
export default class Usuario {
  ...
  presentation() {
    ...
    const escolhaFinanciamento = prompt('Qual o tipo de financiamento? Escolha uma das opções acima: ');

    if(!escolhaFinanciamento ||
      (escolhaFinanciamento.toUpperCase() != 'CASA' &&
      escolhaFinanciamento.toUpperCase() != 'APARTAMENTO' &&
      escolhaFinanciamento.toUpperCase() != 'TERRENO')) return false;
  ...
  getPrazoFinanciamento(): number {
    while(true) {
      try {
        ...
        if(Number(value) > 100) {
          const check: string = prompt(`Tem certeza que o prazo de financiamento anual é ${Number(value)} anos? S/N  `);
  ...
  getTaxaJuros(): number {
    while(true) {
      try{
        ...
        if(Number(value) > 10_000) throw new Error('Valor da taxa de juros excede o limite maximo permitido.')
  ...
  FinCasa() {
    while(true) {
      try {
        ...
        let desconto = prompt('Deseja aplicar desconto de ate 100 reais nas parcelas do Financiamento? S/N')

        let valorDesconto = 0;
        if(desconto.toUpperCase() == 'S'){
          valorDesconto = Number(prompt('Qual o valor? '));

          if(isNaN(valorDesconto) || valorDesconto < 0 || valorDesconto > 100) throw new Error('O desconto deve ser entre 0 e 100 reais.')
        }
  ...
  FinTerreno() {
    while(true) {
      try {
        ...
        console.log(zona);
        if(!zona || (zona.toUpperCase() != 'RESIDENCIAL' && zona.toUpperCase() != 'COMERCIAL')) throw new Error('Digite um dos valores solicitados');
